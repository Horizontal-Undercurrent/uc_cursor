...
function s2w(camPos: Vector3, relX: number, relY: number) {
  ...
  let camRight = Utils.rotationToForward(rotRight).sub(
    Utils.rotationToForward(rotLeft)
  );
  let camUp = Utils.rotationToForward(rotUp).sub(
    Utils.rotationToForward(rotDown)
  );
